...
// Optional Indian Proxy configuration (SOCKS5/HTTP/auto to bypass Geo-Blocking on direct CDN)
// Set to "auto" to enable our Self-Healing, Auto-Scraping, and Parallel-Testing Proxy Engine!
const INDIAN_PROXY = process.env.INDIAN_PROXY || "auto";
let activeAutoProxy = "";
let isHuntingProxy = false;

function huntActiveAutoProxy() {
    if (isHuntingProxy) return;
    isHuntingProxy = true;
    console.log("[Auto Proxy] Starting background hunt for active Indian ISP/Resi proxies...");

    const urlStr = "https://api.proxyscrape.com/v2/?request=getproxies&protocol=http&timeout=10000&country=IN&ssl=all&anonymity=all";
    
    // Fetch proxies list directly (force bypass our own proxy loop for the proxy API!)
    const parsedApi = url.parse(urlStr);
    const optionsApi = {
    ...
    https.request(optionsApi, (res) => {
        let rawData = '';
        res.on('data', chunk => rawData += chunk);
        res.on('end', () => {
            const proxies = rawData.split('\n').map(p => p.trim()).filter(Boolean);
            if (proxies.length === 0) {
                console.error("[Auto Proxy] Downloaded proxy list is empty.");
                isHuntingProxy = false;
                return;
            }

            console.log(`[Auto Proxy] Found ${proxies.length} Indian proxies. Parallel testing for Geo-Block bypass...`);
            let workingProxy = "";
            let index = 0;

            const testTarget = "https://jiotvmblive.cdn.jio.com/bpk-tv/Nick_Ben_MOB/WDVLive/index.mpd";

            async function worker() {
                while (index < proxies.length && !workingProxy) {
                    const proxyIp = proxies[index++];
                    const proxyUrl = `http://${proxyIp}`;
                    
                    try {
                        const parsedTarget = url.parse(testTarget);
                        const proxyParsed = url.parse(proxyUrl);
                        const client = proxyParsed.protocol === 'https:' ? https : http;
                        const options = {
                            hostname: proxyParsed.hostname,
                            port: proxyParsed.port,
                            path: encodeURI(testTarget),
                            method: 'HEAD',
                        ...
                        await new Promise((resolve, reject) => {
                            const req = client.request(options, (pRes) => {
                                if (pRes.statusCode !== 451 && pRes.statusCode < 500) {
                                    workingProxy = proxyUrl;
                                    resolve();
                                } else {
                                    reject(new Error(`Blocked: ${pRes.statusCode}`));
                                }
                            });
                            req.setTimeout(3500, () => {
                                req.destroy();
            ...
            Promise.all(workers).then(() => {
                if (workingProxy) {
                    activeAutoProxy = workingProxy;
                    console.log(`[Auto Proxy Success] Enforced working Indian ISP/Resi proxy: ${activeAutoProxy}`);
                } else {
                    console.warn("[Auto Proxy Warning] No Indian proxy successfully bypassed Jio-Block in this cycle. Retrying in 30 seconds.");
                    setTimeout(huntActiveAutoProxy, 30000);
                }
                isHuntingProxy = false;
            }).catch(() => {
                isHuntingProxy = false;
        ...
        console.error("[Auto Proxy Error] Failed to download public proxy list:", err.message);
        isHuntingProxy = false;
    }).end();
}

// Start auto-proxy hunting loop (re-check every 30 minutes to refresh IPs)
...
function fetchUrl(targetUrl, headers = {}, redirectCount = 0) {
    ...
    return new Promise((resolve, reject) => {
        ...
        let options = {
            ...
            port: parsed.port,
            path: parsed.path ? encodeURI(parsed.path) : '',
            method: 'GET',
            rejectUnauthorized: false,
        ...
        req.on('error', (err) => {
            if (INDIAN_PROXY === 'auto' && activeAutoProxy) {
                console.warn(`[Auto Proxy Error] Active proxy ${activeAutoProxy} failed with: ${err.message}. Re-hunting fresh proxy...`);
                activeAutoProxy = "";
                huntActiveAutoProxy();
            }
            reject(err);
